...
exports.handler = async function http(req) {
	const { user, repo, prPlusFormat } = req.pathParameters;
	const {
		pr,
		format = 'html',
	} = prPlusFormat.match(/^(?<pr>[1-9][0-9]+)(?:\.(?<format>html|json))?$/).groups;
	...
	result = execSync(`node ${path.relative(process.cwd(), path.join(__dirname, './check-form'))} ${user}/${repo} ${sha}`, {
	...
	return {
		...
		body: formatters[format]({ user, repo, pr, sha, result, success })
	};
};
